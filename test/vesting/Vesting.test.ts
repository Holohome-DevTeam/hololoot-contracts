...
import { Vesting, Hololoot, ERC20Mock } from "../../typechain";
...
describe("Vesting", () => {
  ...
  describe("onlyOwner", () => {
    it("should revert if restricted function's caller is not owner", async () => {
      ...
      await expect(vesting.connect(alice).massAddHolders([], [], [], 1, 2)).to.be.revertedWith("caller is not the owner");
  ...
  describe("whenNotLocked", () => {
    it("should revert if function is locked", async () => {
      await vesting.lock();
      await expect(vesting.massAddHolders([], [], [], 1, 2)).to.be.revertedWith("Lockable: locked");
    });
  });

  describe("massAddHolders", () => {
    it("should revert with incorrect arrays lengths", async function () {
      await expect(vesting.massAddHolders([alice.address, bob.address], [100], [1000], 1, 2)).to.be.revertedWith("data size mismatch");
      await expect(vesting.massAddHolders([alice.address], [100, 100], [1000], 1, 2)).to.be.revertedWith("data size mismatch");
      await expect(vesting.massAddHolders([alice.address, bob.address], [100], [1000, 1000], 1, 2)).to.be.revertedWith("data size mismatch");
      await expect(vesting.massAddHolders([alice.address], [100], [1000, 1000], 1, 2)).to.be.revertedWith("data size mismatch");
    });

    it("should revert when endDate is before startDate", async function () {
      await expect(vesting.massAddHolders([alice.address], [100], [1000], 2, 1)).to.be.revertedWith("startDate cannot exceed endDate");
    });

    it("should revert when user address is 0", async function () {
      await expect(vesting.massAddHolders([ZERO_ADDRESS], [100], [1000], 1, 2)).to.be.revertedWith("user address cannot be 0");
    });

    it("should allow to add single vesting", async function () {
      await expect(vesting.massAddHolders([alice.address], [100], [1000], 1, 2))
        .to.emit(vesting, "Vested")
        .withArgs(alice.address, 1000, 2);
    });

    it("should allow to add multiple vestings", async function () {
      await expect(vesting.massAddHolders([alice.address, bob.address, carol.address], [100, 200, 300], [1000, 2000, 3000], 150, 350))
        .to.emit(vesting, "Vested")
    ...
    it("should correctly track totalVested", async function () {
      await vesting.massAddHolders([alice.address, bob.address, carol.address], [100, 200, 300], [1000, 2000, 3000], 150, 350);

      let totalVested = await vesting.totalVested();
      expect(totalVested).to.be.equal(6000);

      await vesting.massAddHolders([alice.address, bob.address, carol.address], [100, 200, 300], [2000, 5000, 8000], 150, 350);
  ...
  describe("claim", () => {
    ...
    it("should revert if nothing to claim when before start time", async function () {
      await vesting.massAddHolders([alice.address], [100], [1000], now.add(duration.days(1)), now.add(duration.days(2)));
    ...
    it("should claim startTokens when block timestamp = startDate", async function () {
      ...
      await vesting.massAddHolders([alice.address], [100], [1000], now.add(3), now.add(duration.days(2)));
    ...
    it("should claim correctly every 100 sec with minimal amount", async function () {
      ...
      await vesting.massAddHolders([alice.address], [1], [11], now.add(3), now.add(1003));
    ...
    it("should claim correctly every 100 sec with normal amount", async function () {
      ...
      await vesting.massAddHolders([alice.address], [getBigNumber(5000)], [getBigNumber(55000)], now.add(3), now.add(1003));

      await advanceTimeAndBlock(300);
      await expect(vesting.connect(alice).claim()).to.emit(vesting, "Claimed").withArgs(alice.address, getBigNumber(20000));
    ...
    it("should claim correctly every 100 sec with totalSupply", async function () {
      ...
      await vesting.massAddHolders([alice.address], [getBigNumber(5000000)], [getBigNumber(25000000)], now.add(3), now.add(1003));
  ...
  describe("getClaimable", () => {
    it("should return claimable for given parameters", async function () {
      ...
      await vesting.massAddHolders([alice.address], [123451], [12345611], timestamp.add(duration.days(1)), timestamp.add(duration.days(7))); // 100 per day

      await vesting.massAddHolders([bob.address], [2345673], [23456783], timestamp.add(duration.days(2)), timestamp.add(duration.days(13))); // 100 per day

      await vesting.massAddHolders([carol.address], [3456789], [34567890], timestamp.add(duration.days(4)), timestamp.add(duration.days(27))); // 100 per day
  ...
  describe("getAllClaimable", () => {
    ...
    it("should return all claimable for given address", async function () {
      ...
      await vesting.massAddHolders([alice.address], [123451], [12345611], timestamp.add(duration.days(1)), timestamp.add(duration.days(7)));

      await vesting.massAddHolders([alice.address], [2345673], [23456783], timestamp.add(duration.days(2)), timestamp.add(duration.days(13)));

      await vesting.massAddHolders([alice.address], [3456789], [34567890], timestamp.add(duration.days(4)), timestamp.add(duration.days(27)));
  ...
  describe("recoverErc20", () => {
    it("should revert if token is SNP", async function () {
      await expect(vesting.connect(alice).recoverErc20(token.address)).to.be.revertedWith("not allowed");
    });
    ...
    it("should revert if nothing to recover", async function () {
      ...
      await expect(vesting.connect(alice).recoverErc20(erc20.address)).to.be.revertedWith("nothing to recover");
    });
    ...
    it("should correctly recover ERC20 to owner address", async function () {
      ...
      await expect(vesting.connect(alice).recoverErc20(erc20.address))
        .to.emit(erc20, "Transfer")
        .withArgs(vesting.address, deployer.address, 1000);
